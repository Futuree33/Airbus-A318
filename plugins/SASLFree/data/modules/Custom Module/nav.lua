...
local function draw_arc_page()
    sasl.gl.drawRotatedTexture(arc_tape_2, 0 - get(heading), 8.8, -90, 550, 550)
end
...
function draw()
    ...
    elseif get(curr_nd_mode) == 1 then 
        sasl.gl.drawTexture(nd_overlay_sec, 22, -44, 500, 500)
        sasl.gl.drawTexture(miniplane, 261, 155, 50, 50)
        sasl.gl.resetClipArea()
        draw_arc_page()
        draw_overlay_text()
    end 
